...
def main(args):
    ...
    for cnt, (train_index, test_index) in enumerate(kf.split(data.numpy())):
        X_train, X_test = data.numpy()[train_index], data.numpy()[test_index]
        y_train, y_test = label.numpy()[train_index], label.numpy()[test_index]
        ...
        # Start training
        best_acc = training(args, train_loader, val_loader, model, device, str(cnt))
        kf_acc.append(best_acc)
        
    print('mean', np.mean(kf_acc))
    print('std', np.std(kf_acc))
    print(kf_acc)
